...
class TFModel:
    ...
    def predict(self, inputs, tf_nms=False, agnostic_nms=False, topk_per_class=100, topk_all=100, iou_thres=0.45,
        ...
        if tf_nms:
            ...
            # remove boxes out of range
            top_indices = tf.where(tf.math.greater(boxes[:, 0], 0))
            new_boxes = tf.gather(boxes, top_indices)[:, 0, :]
            res = tf.gather(res, top_indices)[:, 0, :]

            bottom_indices = tf.where(tf.math.less(new_boxes[:, 2], 1))
            new_boxes = tf.gather(new_boxes, bottom_indices)[:, 0, :]
            res = tf.gather(res, bottom_indices)[:, 0, :]

            boxes = xywh2xyxy(res[..., :4])
            probs = res[:, 4:5]
            classes = res[:, 5:]
            ...
            if not agnostic_nms:
                ...
                ind = tf.image.non_max_suppression(
                    boxes, scores_, 10, 0.25, 0.25)
                nmsed_boxes = tf.expand_dims(tf.gather(boxes, ind), axis=0)
                nmsed_scores = tf.cast(tf.reshape(tf.gather(scores_, ind), (1, -1)), tf.float32)
                nmsed_classes = tf.cast(tf.reshape(tf.gather(classes_, ind), (1, -1)), tf.float32)
                valid_detections = tf.cast(tf.shape(ind), tf.float32)
                nms = (nmsed_boxes, nmsed_classes, nmsed_scores, valid_detections)
